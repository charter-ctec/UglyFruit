...
class MySSH:
    ...
    def run(self, cmd, input_data=None, timeout=20):
        '''
...
def getcreds(userid, field = 'username', dbname = 'rcusers.sqlite'):
    try:
        con = sqlite3.connect(dbname)

        with con:
#'row_factory' - allows column access by name 'columname' instead of just number
            con.row_factory = sqlite3.Row
            cur = con.cursor()
            sqlquery="SELECT * FROM ucreds where userid = " + str(userid)
            ...
            try:
                ...
                #print("Record #: " + str(procrecrods) + " " + tmpchassisid, row['DeviceName'], row['Vendor'], row['RemoteIPAddress'], row['MachineType'], row['CLIAccess'])
            except:
...
def writedump(config,hostname,adir = "cfg"):
    print(config)
# Pad day/month dig to 2 char (add 0 when needed)
# needed for sorting
    ...
    #pdb.set_trace()
    flout = './' + adir + '/' + hostname + '.txt'
    flarcout = './' + adir + '/archive/' + hostname + '-'+ tstamp + '.txt'
    print("writing to file ... " + flout)
    try:
        #for eachline in rtlist:
         #   buffer = buffer + str(eachline) + '\n'
        target = open(flout, 'w')
        target.write(config)
        target.close()
        arctarget = open(flarcout, 'w')
        arctarget.write(config)
        arctarget.close()
    except:
        print('Unable to save to main file or archive: ' + flout)
        print('Error:', sys.exc_info()[1])
        print(traceback.format_exc())
# end of writeconf proc
...
if __name__ == '__main__':
    ...
    try:
        ...
        print 'rcmd.py syntax error - command line usage --> python rcmd <db-user-id> <host-ip> <savetofile> <savedir> "cli cmd"'
        sys.exit()
    # Access variables.
    #hostname = '147.105.0.30'
    hostname = hip
    port = 22
    username = getcreds(userid)
    password = getcreds(userid,'password')
    sudo_password = password  # assume that it is the same password

    if debug:
        print("Connecting to: " + hip + " as: " + username + " output-to: " + hfile + " cmd: '" + hcmd + "'")
        #ipdb.set_trace()
    ...
    def run_cmd(cmd, indata=None):
        ...
        print '=' * 64
        #print '%s' % (output)
        #write to file and archive !!!!!!
        writedump(output,hfile,hdir)
